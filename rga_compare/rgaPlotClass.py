...
class Plot(pg.PlotWidget):
    ...
    def set_axis_limits(self):
        ...
        padding_x = (self.x_value_max - self.x_value_min) * 0.02  # 2% padding
        padding_y = (self.y_value_max - self.y_value_min) * 0.02

        # Sets x axis limits
        view_box.setLimits(
            xMin=(self.x_value_min - padding_x),
            xMax=(self.x_value_max + padding_x),
        )

        if self.log_mode:
            # Sets log y axis limits (accounts for the fact the Pyqtgraph takes the 10^x of limits given)
            y_min = np.log10(self.y_value_min)
            y_max = np.log10(self.y_value_max)
            view_box.setLimits(yMin=y_min, yMax=y_max)
        else:
            # Sets linear y axis limits
            view_box.setLimits(yMin=self.y_value_min - padding_y, yMax=self.y_value_max + padding_y)
